...
namespace AudioMesh
{
    ...
    public class AudioSampler : MonoBehaviour
    {
        ...
        public bool useAudioListener = false; //sample from the AudioListener instead of AudioSource components
        ...
        public float[] GetAudioSamples(int audioSourceIndex)
        {
            if (!audioSources[audioSourceIndex].mute) // if not muted
            {
                ...
                if (useAudioListener)
                {
                    AudioListener.GetOutputData(samples,0);// .GetOutputData(samplesToTake, 0); // grab samples!
                }
                else
                {
                    samples = audioSources[audioSourceIndex].GetOutputData(samplesToTake, 0); // grab samples!
                }
        ...
        public float GetRMS(int audioSourceIndex)
        {
            if (!audioSources[audioSourceIndex].mute)
            {
                ...
                if (useAudioListener)
                {
                    AudioListener.GetOutputData(audioSamples, 0);// .GetOutputData(samplesToTake, 0); // grab samples!
                }
                else
                {
                    audioSamples = audioSources[audioSourceIndex].GetOutputData(samplesToTake, 0); // grab samples!
                }
